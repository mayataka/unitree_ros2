...
UnitreeGazeboSystem::configure(const hardware_interface::HardwareInfo & actuator_info)
{
...
hardware_interface::return_type UnitreeGazeboSystem::start()
{
  status_ = hardware_interface::status::STARTED;
  return hardware_interface::return_type::OK;
}

hardware_interface::return_type UnitreeGazeboSystem::stop()
{
  ...
  for (unsigned int j = 0; j < this->dataPtr->joint_names_.size(); j++) {
    if (this->dataPtr->sim_joints_[j]) {
      if (this->dataPtr->joint_control_methods_[j] & EFFORT) {
        const double tau_feedforward = this->dataPtr->joint_effort_cmd_[j];
        const double pos_feedback = 
          this->dataPtr->Kp_cmd_[j] * (this->dataPtr->joint_position_cmd_[j] - this->dataPtr->joint_position_[j]);
        const double vel_feedback = 
          this->dataPtr->Kd_cmd_[j] * (this->dataPtr->joint_velocity_cmd_[j] - this->dataPtr->joint_velocity_[j]);
